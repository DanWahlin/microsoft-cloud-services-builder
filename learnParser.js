        ...
        if (service.learnContent) {
            for (let learnContent of service.learnContent) {
                if (learnContent.type === 'path' && !learnContent.modules) {
                    getHtml(learnContent.url, service, learnContent); 
                }
            }
        }
    }
...
function getHtml(url, service, learnContent) {   
    var options = {
        url: url,
        header: {
    ...
    request(options, function (err, resp, body) {
        parseData(body, service, learnContent);
    });
}
...
function parseData(html, service, learnContent) {
    ...
    console.log(service.name, learnContent.modules);
}
